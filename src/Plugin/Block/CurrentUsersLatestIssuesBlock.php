...
/**
 * Provides a 'Current User's Latest Issues Block' block.
 *
 * @Block(
 *   id = "current_users_latest_issues_block",
 *   admin_label = @Translation("Current User's Latest Issues Block"),
 * )
 */
class CurrentUsersLatestIssuesBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    $issues = $this->getUserAssignedIssues($current_user);
   ...
   */
  protected function getUserAssignedIssues($userId) {
    // To retrieve the latest 3 issues assigned to the current user.
    $query = $this->entityTypeManager->getStorage('node')
      ->getQuery()
      ->condition('type', 'issue')
      ->condition('field_assignee.target_id', $userId)
      ->sort('created', 'DESC')
      ->range(0, 3)
      ->accessCheck(FALSE);
